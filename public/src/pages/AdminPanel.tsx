      ...
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            Zarządzaj produktami, galerią i ustawieniami sklepu
          </p>
        </div>
              ...
              <div className="flex justify-between items-center mb-6">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    {gallery.length}{' '}
                    {gallery.length === 1 ? 'zdjęcie' : gallery.length < 5 ? 'zdjęcia' : 'zdjęć'} w
                    galerii
                  </p>
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <Input
                      ...
                      onBlur={(e) =>
                        updateSettingMutation.mutate({ key: 'siteName', value: e.target.value })
                      }
      ...
      {product.images && product.images.length > 0 && (
        <div>
          <Label className="text-sm font-medium">
            Дополнительные фото ({product.images.length})
          </Label>
          ...
          <div className="mt-3 space-y-3">
            <div className="relative aspect-square max-w-xs border rounded-md p-2">
              <img
                ...
                className="w-full h-full object-cover rounded-md"
              />
              <Badge className="absolute top-3 left-3">Превью</Badge>
            </div>
...
function ProductForm({ product, onClose }: { product: Product | null; onClose: () => void }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isNaN(priceNum) || priceNum < 0) {
      ...
      toast({
        title: 'Błąd',
        description: 'Podaj prawidłową cenę (min 0)',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (isNaN(stockNum) || stockNum < 0) {
      ...
      toast({
        title: 'Błąd',
        description: 'Podaj prawidłowy stan magazynowy',
        variant: 'destructive',
      });
      return;
    }
