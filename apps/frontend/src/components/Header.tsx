...
export function Header({ cartItemCount = 0 }: { cartItemCount?: number }) {
  ...
  const siteName = getSetting("siteName") || "BHP PERFECT";
        ...
        <div className="flex items-center justify-between px-3 sm:px-6 py-3 sm:py-4 gap-2 sm:gap-4">
          ...
          >
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold tracking-tight">
              {siteName.includes(" ") ? (
                <>
                  <span className="text-black">{siteName.split(" ")[0]}</span>{" "}
                  <span className="text-primary transition-colors">{siteName.split(" ").slice(1).join(" ")}</span>
                </>
              ) : (
